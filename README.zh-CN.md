...
当 `maxConcurrentRequests > 0` 时，插件还会限制每个路由同时在飞的请求数。通过了 RPM 检查但发现并发槽已满的请求会进入 FIFO 槽位队列，前一个请求释放的瞬间即被授予槽位 —— 全程无轮询，进入顺序严格保持到达顺序。这可以防止多个并行 agent 或子 agent 单独突破 RPM 限制但合计压垮供应商。`0` = 不限并发（仅 RPM 限流）。
