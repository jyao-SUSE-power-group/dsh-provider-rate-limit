...
const Config = z.object({
  ...
  /** Fallback cooldown window (ms) when the upstream 429 carries no Retry-After. */
...
async function* runNext(next, bucket, cfg) {
  for await (const ev of next()) {
    if (
      ...
      const ms =
        Number.isFinite(retryMs) && retryMs > 0 ? retryMs : (cfg.backoffMs ?? 30000);
      bucket.cool(ms);
    }
    yield ev;
  }
...
async function apply(ctx, config) {
  ...
  ctx.on(
    ...
    async function* (options, next) {
      ...
      if (waitMs === 0) {
        yield* runNext(next, bucket, effective());
        return;
      }
