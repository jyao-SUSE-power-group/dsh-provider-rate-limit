...
async function apply(ctx, config) {
  ...
  ctx.on(
    ...
    async function* (options, next) {
      ...
      // rpm === 0 means "unlimited": pass through without reserving a slot, so
      // the route is neither throttled nor counted in the stats. This also
      // sidesteps a 0-rate bucket (intervalMs = Infinity).
      if (rpm === 0) {
        yield* next();
        return;
      }
