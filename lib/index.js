 ...
 * registers `llm-rate-limit` so the web UI can edit it. The composition entry
...
const name = "llm-rate-limit";
const inject = ["llm"];
const SETTINGS_NAMESPACE = "llm-rate-limit";
...
const CONFIG_ROUTE = "/api/llm-rate-limit.config";
...
function registerConfigRoute(ctx) {
  ctx.inject(["webServer"], (wctx) => {
    ...
    wctx.effect(() => dispose, "llm-rate-limit: config route");
  });
}

const IDENTITY_PATCH_KEY = Symbol.for("dsh-plugin.llm-rate-limit.identityFetch");
...
async function apply(ctx, config) {
  ...
  if (disposeIdentity) ctx.effect(() => disposeIdentity, "llm-rate-limit: identity fetch patch");
