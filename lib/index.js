 ...
 * registers `provider-rate-limit` with the host settings provider so the web UI
 * can edit it. The composition entry
...
const Config = z.object({
   ...
   */
  maxBackoffMs: z.number().min(0).default(60000),
  /**
...
class TokenBucket {
  constructor(capacity, perSecond) {
    ...
    this.activeCount = 0;
    // FIFO queue of waiters for a concurrency slot. Release hands the freed
    // slot to the queue head, so admission order stays strict without polling.
    this.slotWaiters = [];
    // Real-time gauge: number of requests currently waiting in a delay (RPM
    // queue). Unlike the cumulative `waited` counter this reflects the live
    // queue depth so operators can see backpressure at a glance.
    this.queuedNow = 0;
    // Stats counters.
    this.reserved = 0;
    this.waited = 0;
...
/**
 * Queue-aware delay: runs `delay` while tracking the live queue depth on the
 * bucket so stats expose the current number of requests waiting in the RPM
 * queue, not just the cumulative `waited` counter. Always decrements in a
 * finally so an aborted wait can't leak the gauge.
 */
async function queueDelay(bucket, ms, signal) {
  bucket.queuedNow++;
  try {
    return await delay(ms, signal);
  } finally {
    bucket.queuedNow--;
  }
}
...
/**
 * Claim one concurrency slot, FIFO without polling: slots free up either
 * immediately (under the cap) or the caller joins the route's waiter queue and
 * is granted exactly when a predecessor releases. Resolves true once the slot
 * is held (releaseConcurrencySlot is then owed) and false if the signal
 * aborted first.
 */
function acquireConcurrencySlot(bucket, max, signal) {
  if (bucket.activeCount < max) {
    bucket.activeCount++;
  ...
  return new Promise((resolve) => {
    ...
    waiter.cancel = () => {
      const idx = bucket.slotWaiters.indexOf(waiter);
      if (idx !== -1) bucket.slotWaiters.splice(idx, 1);
      resolve(false);
    };
    bucket.slotWaiters.push(waiter);
    signal?.addEventListener("abort", waiter.cancel, { once: true });
  });
}

/**
 * Give up a concurrency slot. The freed capacity is handed directly to the
 * queue head (FIFO hand-off); only when nobody waits does the in-flight count
 * drop, so an arriving request between release and grant can't jump the queue.
 */
function releaseConcurrencySlot(bucket) {
  const waiter = bucket.slotWaiters.shift();
  if (waiter) {
...
function ulid() {
  ...
  // 80 bits of randomness = 10 bytes, decoded big-endian and re-encoded as 16
  // Crockford chars (5 bits each). Masking each byte with & 31 would carry only
  // 50 bits of entropy; full 80-bit decoding matches the standard ULID layout.
  ...
  for (let i = 0; i < ULID_RANDOM_LEN; i++) {
    ...
    const chunk =
      off <= 3
        ? (randBytes[byteIdx] >> (3 - off)) & 31
        : ((randBytes[byteIdx] << (off - 3)) | (randBytes[byteIdx + 1] >> (11 - off))) & 31;
    out += ULID_ALPHABET[chunk];
  }
  return out;
}
...
function installIdentityPatch(effective) {
  ...
  // A lingering state whose wrap no longer occupies globalThis.fetch was
  // superseded by an external swap (test suites, hot reload) — its restore
  // chain is severed and its captured native is stale. Discard it and rebuild
  // from whatever currently holds the fetch slot.
  if (state && globalThis.fetch !== state.wrap) state = void 0;
...
/**
 * Install the canonical optional-settings consumer wiring. DSH >= 0.1.2-alpha.2
 * exposes SettingsProvider#installSection; earlier versions shipped the same
 * wiring as a module-level installSettingsSection that no longer exists — a
 * named import of it crashes the whole plugin at load time. The fallback
 * reimplements the identical contract against the provider's register()
 * surface, so the plugin loads on both generations.
 */
...
async function apply(ctx, config) {
  ...
  const statsService = {
    ...
    getAggregateStats() {
      ...
      for (const entry of buckets.values()) {
        ...
        queuedNow += entry.bucket.queuedNow;
      }
      return {
        ...total,
        queuedNow,
  ...
  // SSE connection pool: browser clients connect to /events for push updates.
  const sseClients = new Set();
  // Broadcast coalescing: counter bursts (a queue draining, parallel requests)
  // serialize one snapshot per 15 ms window instead of one per event. The
  // trailing edge guarantees the newest snapshot still lands after the burst.
  let broadcastScheduled = false;
  function broadcastStats() {
    if (sseClients.size === 0 || broadcastScheduled) return;
  ...
  // Server-Sent Events endpoint: browser clients hold a long-lived connection
  // and receive a fresh stats snapshot whenever counters move, instead of
  // polling the .stats route on a fixed timer. Plain HTTP flush per frame —
  // no Resumable/stream mux, just a raw response we write to directly.
  ctx.inject(["webServer"], (wctx) => {
    const dispose = wctx.webServer.register({
      kind: "exact",
      path: "/api/provider-rate-limit.events",
      handler: (req, res) => {
        if (req.method !== "GET") {
          writeJson(res, 405, { ok: false, code: "method-not-allowed" });
          return;
        ...
        // Send an initial frame so the client has data instantly on connect.
        res.write(`data: ${JSON.stringify({ aggregate: statsService.getAggregateStats(), routes: statsService.getAllStats() })}\n\n`);
        sseClients.add(res);
        ...
        // Prevent the event loop from treating the idle socket as finished.
    ...
    wctx.effect(() => {
      ...
      sseClients.clear();
      return dispose;
    }, "provider-rate-limit: events route");
  });
    ...
    async function* (options, next) {
      ...
      if (waitMs > 0) {
        if (
          ...
          (await queueDelay(bucket, waitMs, options.signal))
        ) {
      ...
      // in-flight slot even if RPM capacity is available. Slots are granted
      // FIFO on release — no polling; the abort signal breaks the wait.
      const maxConcurrent = cfg.maxConcurrentRequests ?? 0;
      if (maxConcurrent > 0) {
        if (!(await acquireConcurrencySlot(bucket, maxConcurrent, options.signal))) return;
        // Grant raced an abort: hold-then-release keeps the slot count exact.
        if (options.signal?.aborted) {
          releaseConcurrencySlot(bucket);
          return;
        }
      }
      ...
      try {
        ...
        if (maxConcurrent > 0) releaseConcurrencySlot(bucket);
        broadcastStats();
      }
    },
