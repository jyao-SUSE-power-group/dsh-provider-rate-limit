window.__ModuleLoader__.load({
	...
	function draftIssues(draft) {
		...
		const zh = {
			...
			backoffMsHint: "上游未返回 Retry-After 时使用的兜底冷却时长",
			rowsTitle: "按路由限速",
			provider: "供应商",
			model: "模型",
			rpm: "RPM",
			...
			stateOff: "已停用",
			failed: "保存失败",
			invalidNumbers: "无法保存:速率不能为负数(0 表示不限速),突发需为不小于 1 的整数,最长等待不能为负数",
		...
		const en = {
			...
			backoffMsHint: "Fallback cooldown when the upstream sends no Retry-After",
			...
			stateOff: "Disabled",
			failed: "Save failed",
			invalidNumbers: "Cannot save — rates cannot be negative (0 = unlimited), burst a whole number >= 1, and max wait cannot be negative",
					...
					children: [
						jsxs("div", {
							...
							children: [
								...
								jsx("span", { children: `${t("statsRejected")} ${agg.rejected || 0} \u00b7 ${t("statsWaited")} ${agg.waited || 0} \u00b7 ${t("statsAvgWait")} ${avg} \u00b7 ${t("statsTotal")} ${agg.reserved || 0} \u00b7 ${t("statsRoutes")} ${routeCount}` })
							]
						}),
