window.__ModuleLoader__.load({
	...
	const NS = "llm-rate-limit";
	const CONFIG_URL = "/api/llm-rate-limit.config";
	...
	const IDENTITY_PRESETS = [
		...
		const tagId = "dsh-llm-rate-limit/card.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-llm-rate-limit";
		...
		function apply(ctx) {
			...
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "llm-rate-limit: card dictionaries");
