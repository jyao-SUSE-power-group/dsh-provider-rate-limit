	...
	function draftIssues(draft) {
		...
		function apply(ctx) {
			...
			const ComposerStatsLine = () => {
				...
				react.useEffect(() => {
					// dsh-client-store's subscribe is a notify-only callback (no
					// payload); pull the fresh snapshot ourselves, like every
					// in-tree DSH consumer (see model-selection etc.).
					const dispose = controller.statsStore.subscribe(() =>
						setSnap(controller.statsStore.getSnapshot())
					);
					return () => dispose();
				}, []);
