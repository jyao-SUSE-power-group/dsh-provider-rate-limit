window.__ModuleLoader__.load({
	...
	// Package-private RPC registered by the host half; returns { aggregate, routes }.
	const STATS_RPC = "provider-rate-limit.stats";
	...
	function draftIssues(draft) {
		...
		// ---------------------------------------------------------------------------
		// Stats dashboard — live numbers below the config card.
		// ---------------------------------------------------------------------------
		class StatsCardController {
			constructor(host) {
				this.host = host;
			...
			_startPolling() {
				this._fetch();
				this._poll = setInterval(() => this._fetch(), 5000);
			}
			async _fetch() {
				try {
					const result = await this.host.call(STATS_RPC, {});
					this.state = { status: "ready", data: result, error: null };
				} catch (err) {
					this.state = { status: "error", data: null, error: err?.message ?? String(err) };
				}
				this._publish();
			}
			_snapshot() {
				const s = this.state;
				return { status: s.status, data: s.data, error: s.error };
			}
			inject() {
		...
		function StatsCard(props) {
			const snap = props.useSnapshot((s) => s);
			const { t } = props;
			if (snap.status === "loading") {
				return jsxs("div", { className: "rl_statsCard rl_statsLoading", children: [
					jsx("span", { className: "rl_statsTitle", children: t("statsTitle") }),
					jsx("span", { className: "rl_statsHint", children: t("statsLoading") })
				] });
			}
			...
			return jsxs("div", { className: "rl_statsCard", children: [
				jsxs("div", { className: "rl_statsHead", children: [
					jsx("span", { className: "rl_statsTitle", children: t("statsTitle") }),
					jsx("span", { className: "rl_statsBadge", children: agg.routes > 0 ? `${agg.routes}` : "0" })
				] }),
				jsxs("div", { className: "rl_statsGrid", children: [
					jsxs("div", { className: "rl_statItem", children: [
						jsx("span", { className: "rl_statVal", style: agg.rejected > 0 ? { color: "var(--dsw-alias-status-danger)" } : {} }, String(agg.rejected)),
						jsx("span", { className: "rl_statLabel", children: t("statsRejected") })
					] }),
					jsxs("div", { className: "rl_statItem", children: [
						jsx("span", { className: "rl_statVal", children: String(agg.waited) }),
						jsx("span", { className: "rl_statLabel", children: t("statsWaited") })
					] }),
					jsxs("div", { className: "rl_statItem", children: [
						jsx("span", { className: "rl_statVal", children: agg.avgWaitMs > 0 ? `${agg.avgWaitMs}ms` : "—" }),
						jsx("span", { className: "rl_statLabel", children: t("statsAvgWait") })
					] }),
					jsxs("div", { className: "rl_statItem", children: [
						jsx("span", { className: "rl_statVal", children: String(agg.reserved) }),
						jsx("span", { className: "rl_statLabel", children: t("statsTotal") })
					] })
				] }),
				agg.routes > 0 ? jsxs("div", { className: "rl_statsRoutes", children: [
					jsx("span", { className: "rl_statsRouteLabel", children: t("statsRoutes") }),
					...Object.entries(snap.data.routes).slice(0, 3).map(([key, st]) => {
						const [provider, model] = key.split("\u0000");
						return jsxs("span", { className: "rl_statsRouteChip", children: [
							jsx("span", { className: "rl_statsRouteProvider", children: provider || t("allProviders") }),
							model ? jsx("span", { className: "rl_statsRouteModel", children: model }) : null,
							st.rejected > 0 ? jsx("span", { className: "rl_statsRouteReject", children: `×${st.rejected}` }) : null
						] }, key);
					}),
					Object.keys(snap.data.routes).length > 3
						? jsx("span", { className: "rl_statsMore", children: `+${Object.keys(snap.data.routes).length - 3}` })
						: null
				] }) : null
			] });
		}
		...
		function apply(ctx) {
			...
			controller.t = t;
			const statsController = new StatsCardController(host);
			ctx.slots.inject("settings.plugin.item", function* () {
				...
				yield ctx.slots.register({
					name: "settings.plugin.item",
					key: "provider-rate-limit-stats",
					order: 31,
					locale: NS,
					inject: () => statsController.inject()
				}, StatsCard);
			});
		}
